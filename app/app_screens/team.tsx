...
export default function MyTeamScreen({ navigation = null }) {
  ...
  const handleGoBack = () => {
    if (router && typeof router.back === 'function') {
      ...
      console.log("Router back requested, but router is not available");
      if (isAddingMember) {
        setIsAddingMember(false);
      }
    }
    ...
    <SafeAreaView style={{ 
      ...
      {members.length === 0 ? (
        <View style={{ 
          ...
          <Text style={{ color: '#8E8E93', fontWeight: '400', fontSize: 14 }}>No team members yet</Text>
          <TouchableOpacity 
          ...
          {members.map((member) => (
            ...
            >
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 14, color: '#555' }}>{member.email}</Text>
                <Text style={{ fontSize: 14, color: '#555' }}>{member.mobileNumber}</Text>
                <Text style={{ fontSize: 14, color: '#555' }}>{member.address}</Text>
                <Text style={{ fontSize: 14, color: '#555' }}>{member.locations}</Text>
                <Text style={{ fontSize: 14, color: member.isActive ? 'green' : 'gray' }}>
                  {member.isActive ? 'Active' : 'Inactive'} - {member.role}
                </Text>
              </View>
              <TouchableOpacity 
