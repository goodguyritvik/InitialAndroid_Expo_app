...
import { fetchUsers, createUser, updateUser } from '../utils/apiService';
...
export default function AgriDronePilotPage() {
  ...
  // Add explicit types for any implicit any variables
  // For example, if you have a picker or other variables, define their types here
  ...
  const submitForm = async () => {
    ...
    setError('');
    try {
        const createdPilot = await createUser(newPilot);
        setPilots(prev => [...prev, createdPilot as Pilot]);
        resetForm();
        setCurrentView('list');
    } catch (err) {
      setError('Failed to add pilot');
      Alert.alert('Error', 'Failed to add pilot');
    } finally {
      setLoading(false);
    }
  };
  ...
  const togglePilotStatus = async (id: string) => {
    setLoading(true);
    setError('');
    try {
      const pilot = pilots.find(p => p.id === id);
      if (!pilot) throw new Error('Pilot not found');
      const updatedPilot = { ...pilot, active: !pilot.active };
      await updateUser(id, { active: updatedPilot.active });
      setPilots(pilots.map(p => p.id === id ? updatedPilot : p));
    } catch (err) {
      setError('Failed to update pilot status');
      Alert.alert('Error', 'Failed to update pilot status');
    } finally {
      setLoading(false);
    }
  };
